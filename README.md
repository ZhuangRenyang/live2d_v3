...
>
> **改数据源会立即生效**：「添加外部模型源」弹窗里那个「数据源」下拉同时管两件事 ——
> ① 拉外部仓库的 `models.json` 时用哪个源；② **本页左侧列表接下来取模型资源用哪个源**。
> 选完当场换，不用刷新页面（改回「GitHub 原始地址」也是当场换回去）。
> ⚠️ 有个刻意的例外：**已经**在加速源上时，再选另一个加速站**不会**把当前源拽走 ——
> 那一步只更新「兜底用哪个站」的偏好。理由是当前源可能是 raw 挂掉后自动兜底切过去的，
> 拽走它会打断已经跑通的那条连接。想真换回来，先选一次「GitHub 原始地址」再选加速站。
>
...
| `tmp/_probe_extsrc.js` | 129 | 添加外部模型源：＋ 按钮**位置与样式**（绝对定位、规则在顶层未被 `@media` 包住、与 📌 同一行同尺寸且在左侧；手机端与 × 对齐；侧栏直接子元素仍是 4 个）/ 弹窗三个关闭入口（**默认源是 GitHub 原始地址**）/ raw 链接与**加速链接**解析（含各类错误链接）/ 源下拉（10 项 = GitHub + 8 个内置加速站 + 自定义）/ **自定义加速地址行**与解析预览联动 / **真走加速前缀**（服务端看见 URL 里嵌了 `https://raw.githubusercontent.com/…`）/ 源切换 github↔accel / **改数据源后左侧列表立即换源**（切加速站→基址当场变成加速前缀形态、候选表重排、列表头标签跟着变、**服务端计数证明模型资源真的从加速地址取**（且这条路径上一条 raw 请求都没有）、换个模型真能载入；切回 GitHub 对称恢复；已在加速态时换另一个站**不拽走当前源**只更新兜底偏好；选到「自定义…」还没填地址时不拿脏值换源；本仓库模型与外部源两条路都覆盖）/ **侧栏层级（源 → 文件夹 → 模型：子层嵌在源层内、根目录模型不套子层、计数为子树总数、✕ 只在源层、默认折叠、点开后真的可见、折叠状态按全路径记、切换模型时逐层展开）** / 同 owner 重复添加用最新清单替换 / 移除整组 / **端到端**（假远端清单经真 fetch 走通，成功后弹窗自动关；选 github 时落点确实是 raw）/ **记住填过的链接**（真 reload 后仍预填，含源下拉选择与自定义加速地址；**无记忆时默认 GitHub**；清空 URL 即忘但加速站偏好保留）/ **手机端抽屉让位**（从抽屉里点 ＋ 或「本地预览」，抽屉自动收起且真的滑出视口） |
