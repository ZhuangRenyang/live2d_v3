...
Pages 从 `gh-pages` 发布。页面加载模型的**源优先级**（2026-09-24 起）：部署同源 →
`master` 的 GitHub 原始地址（**不用 jsDelivr 了**）→「加速地址」兜底。
`gh-pages` 是壳分支、没有 `models/`，同源会在「验明正身」（见下节）后被自动除名，
实际永远从 GitHub 原始地址起读 —— 和以前一样。
...
> ⚠️ 页面里的模型地址写在 `assets/js/app.js` 顶部的 `rawBaseOf()` 与
> `ACCEL_PRESETS` / `accelerate()`。把仓库改名或换用户名后，改 `REPO_OWNER` / `REPO_NAME`
> 即可（**默认同源优先 → GitHub 原始地址 → 加速地址兜底**；
> 用 `?src=github` / `?src=accel` / `?src=local` 可强制指定，方便排查）。
>
> ⚠️ 加速地址是**前缀代理**形态（`<加速站><原始地址>`，两个 `https://` 连在一起），
> 不是「替换域名」那种。拼错的典型症状是模型全部 404，而清单本身取得到。
>
> **两处都会自动切备源**：① 读 `models.json` 时探到主源不可用；
> ② 模型资源取不到时（例如加速站整段抽风，或者 GitHub 原始地址对批量请求限速，
> 而 20KB 的清单恰好取得到）——
> 这种情况下列表能列出来但每个模型都载入失败，所以载入失败会**再切一次**重试。

## 部署到 Vercel / Netlify / Cloudflare Pages（整仓托管）
...
> 本机起服务时则走相对路径。侧栏标题旁的来源标注会写出当前是 `同源` / `GitHub` / `加速` / 本机
> 以及命中的分支，鼠标悬停能看到完整地址 —— 排查「模型没更新」时先看这里。
>
> **默认同源优先**（整仓部署时 `models/` 就在部署产物里，见「部署到 Vercel」一节）；
> 同源没有模型文件时落到 **GitHub 原始地址**（`raw.githubusercontent.com`，永远是最新的，
> 没有 CDN 缓存），再拉不到时**自动改用加速地址兜底**（用户上次选的那个加速站，
> 没选过则用内置第一个）。追新模型时不会有「刚推上去要等几分钟才出现」的问题。
>
> **改数据源要「确认」才生效**：「添加外部模型源」弹窗里那个「数据源」下拉同时管两件事 ——
> ① 拉外部仓库的 `models.json` 时用哪个源；② **本页左侧列表接下来取模型资源用哪个源**。
>
> ⚠️ 只在弹窗里拨了下拉是**不会**换源的（也不会写进 `localStorage`）—— 你可以放心点开
> 看看每个加速站叫什么。真正换源发生在点「**读取并添加**」那一刻，当场生效、不用刷新页面。
> 点「取消」/「×」/点遮罩关窗则一律**保持原样**（连偏好也不动）。
> 这样区分「用户真的想换」和「只是点着看看」，不会误操作。
> ⚠️ 换站也一样：只要确认过，源就真的换到新站上（哪怕当前已经在别的加速站上）。
>
> **加速地址是「前缀代理」形态**，不是替换域名：
>
> ```
> https://raw.githubusercontent.com/<owner>/<repo>/<branch>/<path>
>   ↓ 套上加速前缀
> https://gh-proxy.org/https://raw.githubusercontent.com/<owner>/<repo>/<branch>/<path>
> ```
>
> 内置了若干常用加速站（`gh-proxy.org` / `ghproxy.net` / `github.dpik.top` / `gh.dpik.top` /
> `ghfile.geekertao.top` / `github.tbap.top` / `ghf.无名氏.top` / `gh.927223.xyz`），
> 也可以在下拉里选「自定义加速地址…」自己粘一个。选过的会被记住（`localStorage`）。
> 用 `?src=github` / `?src=accel` 可强制指定（老写法 `?src=raw` / `?src=cdn` 仍然兼容），
> 排查时更方便。
>
> **分支不用你操心**：GitHub 新建仓库默认分支叫 `main`，老仓库多是 `master`，
> 页面**两个都试**（先 `master` 后 `main`），谁先取到 `models.json` 就用谁。
> 探明分支之后候选源会收窄成「同一分支的 GitHub 原始 → 加速」两项
> （如果命中的是加速地址，收窄后加速排前面；如果一开始就用 `?src=` 强制了某一类，
> 候选表里就只剩那一类），这样「主源半死时换备用源」那唯一一次重试仍然是**换源**而不是换分支。
...
| **添加外部模型源** | 侧栏头「模型列表」右边的 **＋** 按钮。粘一条别人仓库的 `models.json` 链接（原始地址 / 加速地址均可），自动把那个仓库的模型**按用户名 + 它自己的目录层级**加进左侧列表，可一键移除整组。**填过的链接会记住**（刷新后自动预填）。详见「添加外部模型源」一节 |
...
`weiraing ▸ Azue Lane(JP) ▸ aierdeliqi_4`（列表项右侧标一个 `GitHub` / `加速`
...
# ② 加速地址：加速站前缀 + 上面的原始地址（整串拼在一起，不是替换域名）
https://gh-proxy.org/https://raw.githubusercontent.com/<owner>/<repo>/refs/heads/<branch>/models.json
```

**源可切**：对话框里是一个「源」下拉框，**默认是 `GitHub 原始地址`**，下面依次列出内置的
各个加速站，最后一项是「自定义加速地址…」（选中后会多出一行输入框，粘自己的加速前缀）。
页面用同一个 `owner/repo/branch` 按选中的形态重新拼地址。
粘上链接后「解析预览」会实时显示解析出的仓库 / 分支 / 生效源，粘错了当场能看出来
（贴的是加速链接时会**自动认出是哪个加速站**并把它选进下拉，不要求你再选一遍）。
...
- **不持久化**：外部源只活在当前这次会话，刷新页面就没了，下次要用重新加一遍。
  ⚠️ **但填过的链接会记住**：对话框里的输入框、源下拉的选择（含自定义加速地址）会存进
  `localStorage`，刷新网页 / 下次打开自动预填（清空输入框即忘掉 URL，但加速站偏好会留着 ——
  主页面兜底还要用它）。注意记住的只是**地址**，不是「已加载的模型」。
- **同一用户名只保留一份**：重复添加同一个 owner 时，旧的整组被新清单替换，不会叠加。
- **失败自动切源**：先用你选的那个源拉，拉不到自动试另一个（GitHub 原始 ↔ 加速）；两个都拉不到
  才在对话框里报错，并把两个源各自的失败原因一起列出来。
- **外部模型的备用源也是它自己的**：某个外部模型加载失败时，页面切的是**那个外部源自己的**
  GitHub 原始 ↔ 加速，不会动本仓库正在用的源 —— 否则一个外部仓库挂掉会把整个页面的模型来源切走。
...
│   ├── _probe_pagesplit.js      页面/模型分离专项（本机相对路径 + 假 Pages 域名走原始/加速跨源取模型 + master→main 分支回退 + 加速前缀拼接）
│   ├── _probe_extsrc.js         添加外部模型源专项（假域名 + 假远端清单：解析 / 选源与自定义加速 / 切源 / 加层级 / 移除组 / 记住链接 / 手机抽屉让位）
...
>   -addext "subjectAltName=DNS:raw.githubusercontent.com,DNS:gh-proxy.org,DNS:weiraing.github.io,DNS:localhost,IP:127.0.0.1"
> ```
>
...
| 脚本 | 断言数 | 覆盖 |
| --- | --- | --- |
| `tmp/_regress.js` | 97 | 回归：布局 / 全屏 / 播放 / 循环动作 / 速度基准 / 三档舞台主题 / 控制条结构 |
...
| `tmp/_probe_pagesplit.js` | 47 | 页面/模型分离：本机同源走相对路径（回归底线）；假 Pages 域名下**默认同源在前**（候选表 `['', raw@master, raw@main, accel@master, accel@main]`；同源验真失败自动除名后跨源取到 `models.json` + model3 + moc3 + 纹理，外部候选保持原始在前、加速在后）；空格/括号/中文逐段编码；`?src=` 强制选源（`github`/`accel`/`local`，兼容老写法 `raw`/`cdn`）；两个源都挂时降级到内置清单且不白屏；**主源「半死」**（清单取得到、模型资源 403）时自动切到加速源并载入成功；**分支回退**（`master` 取不到时自动试 `main`，并证明候选表已收窄成同一分支的两个源）；**加速前缀拼接**（拼出来的地址形态 / 8 个内置预设 / 解析时剥前缀 / 只写域名也认 / 拒非 raw 链接） |
| `tmp/_probe_checker.js` | 24 | 棋盘底：旧「显示网格」控件已移除、新控件文案正确；**默认就是开**（勾选框 + `S.checkerOn` + `.stage.checker` class 三者一致，首次访问无存储也不能半生效）；勾选后背景真的是 `conic-gradient` 且带 `background-size`；浅/深/黑三档舞台主题下棋盘底色跟着变（CSS 顺序压得住）；取消勾选回到 `radial-gradient`；刷新后按存储恢复 |
| `tmp/_probe_extsrc.js` | 149 | 添加外部模型源：＋ 按钮**位置与样式**（绝对定位、规则在顶层未被 `@media` 包住、与 📌 同一行同尺寸且在左侧；手机端与 × 对齐；侧栏直接子元素仍是 4 个）/ 弹窗三个关闭入口（**默认源是 GitHub 原始地址**）/ raw 链接与**加速链接**解析（含各类错误链接）/ 源下拉（10 项 = GitHub + 8 个内置加速站 + 自定义）/ **自定义加速地址行**与解析预览联动 / **真走加速前缀**（服务端看见 URL 里嵌了 `https://raw.githubusercontent.com/…`）/ 源切换 github↔accel / **改数据源必须「确认」才生效**（⚠️ 走**真实 DOM change** 事件驱动：只拨下拉不换源、不写偏好；点「取消」/「×」关窗一律还原，含「已确认过 A 站 → 改选 B 站 → 取消」要退回 A 而不是被拽到 B；确认后才当场换源 + 落盘；自加地址进「已知加速地址」表也是确认那一刻才写）/ **候选表首位 === 生效源**（源半死兜底后 `cands[0]` 必须跟着 `S_modelsBase` 走 —— 旧行为下兜底只挪了 `S_modelsBase`、候选表没动，于是「明明在用加速，点模型第一跳还打 raw」；反向验证：去掉 promoteModelsBase → J2c 立刻报红）/ **确认后左侧列表立即换源**（切加速站→基址当场变成加速前缀形态、候选表重排、列表头标签跟着变、**服务端计数证明模型资源真的从加速地址取**（且这条路径上一条 raw 请求都没有）、换个模型真能载入；切回 GitHub 对称恢复；已在加速态时选另一个站并确认→真的换过去并重建候选表；选到「自定义…」还没填地址时不拿脏值换源；本仓库模型与外部源两条路都覆盖）/ **侧栏层级（源 → 文件夹 → 模型：子层嵌在源层内、根目录模型不套子层、计数为子树总数、✕ 只在源层、默认折叠、点开后真的可见、折叠状态按全路径记、切换模型时逐层展开）** / 同 owner 重复添加用最新清单替换 / 移除整组 / **端到端**（假远端清单经真 fetch 走通，成功后弹窗自动关；选 github 时落点确实是 raw）/ **记住填过的链接**（真 reload 后仍预填，含源下拉选择与自定义加速地址；**无记忆时默认 GitHub**；清空 URL 即忘但加速站偏好保留）/ **手机端抽屉让位**（从抽屉里点 ＋ 或「本地预览」，抽屉自动收起且真的滑出视口） |
