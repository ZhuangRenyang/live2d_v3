...
| `tmp/_probe_extsrc.js` | 145 | 添加外部模型源：＋ 按钮**位置与样式**（绝对定位、规则在顶层未被 `@media` 包住、与 📌 同一行同尺寸且在左侧；手机端与 × 对齐；侧栏直接子元素仍是 4 个）/ 弹窗三个关闭入口（**默认源是 GitHub 原始地址**）/ raw 链接与**加速链接**解析（含各类错误链接）/ 源下拉（10 项 = GitHub + 8 个内置加速站 + 自定义）/ **自定义加速地址行**与解析预览联动 / **真走加速前缀**（服务端看见 URL 里嵌了 `https://raw.githubusercontent.com/…`）/ 源切换 github↔accel / **改数据源必须「确认」才生效**（⚠️ 走**真实 DOM change** 事件驱动：只拨下拉不换源、不写偏好；点「取消」/「×」关窗一律还原，含「已确认过 A 站 → 改选 B 站 → 取消」要退回 A 而不是被拽到 B；确认后才当场换源 + 落盘；自加地址进「已知加速地址」表也是确认那一刻才写）/ **确认后左侧列表立即换源**（切加速站→基址当场变成加速前缀形态、候选表重排、列表头标签跟着变、**服务端计数证明模型资源真的从加速地址取**（且这条路径上一条 raw 请求都没有）、换个模型真能载入；切回 GitHub 对称恢复；已在加速态时选另一个站并确认→真的换过去并重建候选表；选到「自定义…」还没填地址时不拿脏值换源；本仓库模型与外部源两条路都覆盖）/ **侧栏层级（源 → 文件夹 → 模型：子层嵌在源层内、根目录模型不套子层、计数为子树总数、✕ 只在源层、默认折叠、点开后真的可见、折叠状态按全路径记、切换模型时逐层展开）** / 同 owner 重复添加用最新清单替换 / 移除整组 / **端到端**（假远端清单经真 fetch 走通，成功后弹窗自动关；选 github 时落点确实是 raw）/ **记住填过的链接**（真 reload 后仍预填，含源下拉选择与自定义加速地址；**无记忆时默认 GitHub**；清空 URL 即忘但加速站偏好保留）/ **手机端抽屉让位**（从抽屉里点 ＋ 或「本地预览」，抽屉自动收起且真的滑出视口） |
